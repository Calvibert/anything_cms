...
<?php if (is_array($type)): ?>
    <?php if ($type['type'] === 'autofill'): ?>
        <div class="autocomplete">
            <input id="<?= $type['field'] ?>" type="text" name="<?= $type['field'] ?>" placeholder="<?= Inflector::humanize($type['field']) ?>">
            <script>
                $("#<?= $type['field'] ?>").keypress(function() {
                    setTimeout(
                        () => {
                            var suggestions = [];
                            $.ajax({
                                url: '<?= $this->Url->build(['controller' => $type['field'], 'action' => 'autocomplete']) ?>?key='+$("#<?= $type['field'] ?>").val(),
                                method: 'GET',
                                success: (response) => {
                                    suggestions = JSON.parse(response);
                                    autocomplete(document.getElementById('<?= $type['field'] ?>'), suggestions);
                                }
                            });
                        }, 100
                    );
...
<?php elseif ($type === 'checkbox'): ?>
    ...
    <input type="hidden" id="<?= $field ?>" name="<?= $field ?>" value="false"/>
    <script>
        $("#checkbox-<?= $field ?>").change(function() {
            var state = JSON.parse($("#<?= $field ?>").val());
            console.log(state);
            $("#<?= $field ?>").val(!state);
        })
    </script>
